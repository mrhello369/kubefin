...
func QueryAllClustersCurrentMonthCost(tenantId string) (map[string]*types.ClusterCostsSummary, error) {
	...
	clusterCostSummary := make(map[string]*types.ClusterCostsSummary)
	for clusterId, activeTime := range allClustersActiveTime {
		...
		clusterCostSummary[clusterId] = &types.ClusterCostsSummary{
...
func QueryClusterCurrentMonthCost(tenantId, clusterId string) (*types.ClusterCostsSummary, error) {
	start, end, err := utils.GetCurrentMonthFirstLastDay()
	if err != nil {
		klog.Errorf("Query current time error:%v", err)
		return nil, err
	...
	return &types.ClusterCostsSummary{
