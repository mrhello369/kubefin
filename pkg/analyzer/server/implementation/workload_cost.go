...
func QueryWorkloadCostsWithTimeRange(tenantId, clusterId string,
	start, end, stepSeconds int64, aggregateBy string) (*types.ClusterWorkloadCostList, error) {
	var errs []error
	var err error
	var wg sync.WaitGroup

	var podCosts []*types.ClusterWorkloadCost
	if aggregateBy == types.AggregateByPod || aggregateBy == types.AggregateByAll {
		wg.Add(1)
		go func() {
			defer wg.Done()
	...
	var workloadCosts []*types.ClusterWorkloadCost
	if aggregateBy != types.AggregateByPod {
		wg.Add(1)
		go func() {
			defer wg.Done()
	...
	ret := &types.ClusterWorkloadCostList{ClusterId: clusterId, Items: []*types.ClusterWorkloadCost{}}
	ret.Items = append(ret.Items, podCosts...)
	ret.Items = append(ret.Items, workloadCosts...)
	return ret, nil
}

func queryPodCostsWithTimeRange(tenantId, clusterId string, start, end, stepSeconds int64) ([]*types.ClusterWorkloadCost, error) {
	...
	podWorkloadCost := make(map[string]map[int64]*types.ClusterWorkloadCostDetail)
...
func parsePodTotalCost(podWorkloadCost map[string]map[int64]*types.ClusterWorkloadCostDetail, totalCosts map[string]map[int64]float64) {
	for pod, details := range totalCosts {
		podWorkloadCost[pod] = make(map[int64]*types.ClusterWorkloadCostDetail)
		for timeStamp, v := range details {
			podWorkloadCost[pod][timeStamp] = &types.ClusterWorkloadCostDetail{
				Timestamp: timeStamp,
				TotalCost: v,
...
func parsePodResourceRequest(podWorkloadCost map[string]map[int64]*types.ClusterWorkloadCostDetail,
	...
	for pod, details := range cpuRequest {
		...
		for timeStamp, v := range details {
			if _, ok := item[timeStamp]; !ok {
				item[timeStamp] = &types.ClusterWorkloadCostDetail{
					Timestamp: timeStamp,
					// PodCount is always 1
					PodCount: 1,
				}
...
func parsePodResourceUsage(podWorkloadCost map[string]map[int64]*types.ClusterWorkloadCostDetail,
...
func queryHighLevelWorkloadCostsWithTimeRange(tenantId, clusterId string, start, end, stepSeconds int64, aggregateBy string) ([]*types.ClusterWorkloadCost, error) {
	queryRe := aggregateBy
	if aggregateBy == types.AggregateByAll {
		queryRe = ".*"
	}
	...
	workloadCost := make(map[string]map[int64]*types.ClusterWorkloadCostDetail)
...
func parseHighLevelWorkloadTotalCost(workloadCost map[string]map[int64]*types.ClusterWorkloadCostDetail,
	totalCosts map[string]map[int64]float64) {
	for workload, details := range totalCosts {
		workloadCost[workload] = make(map[int64]*types.ClusterWorkloadCostDetail)
		for timeStamp, v := range details {
			workloadCost[workload][timeStamp] = &types.ClusterWorkloadCostDetail{
				Timestamp: timeStamp,
				TotalCost: v,
			}
		}
...
func parseHighLevelWorkloadPodCount(workloadCost map[string]map[int64]*types.ClusterWorkloadCostDetail,
...
func parseHighLevelWorkloadResourceRequest(workloadCost map[string]map[int64]*types.ClusterWorkloadCostDetail,
...
func parseHighLevelWorkloadResourceUsage(workloadCost map[string]map[int64]*types.ClusterWorkloadCostDetail,
...
func convertClusterWorkloadCostToList(workloadCost map[string]map[int64]*types.ClusterWorkloadCostDetail) []*types.ClusterWorkloadCost {
	ret := []*types.ClusterWorkloadCost{}
	for workloadKey, details := range workloadCost {
		workloadType, namesapce, name := parseWorkloadNamespaceNameType(workloadKey)
		cost := &types.ClusterWorkloadCost{
			Namespace:    namesapce,
			WorkloadName: name,
			WorkloadType: workloadType,
			CostList:     []*types.ClusterWorkloadCostDetail{},
		}
