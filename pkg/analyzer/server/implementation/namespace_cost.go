...
func QueryNamespaceCostsWithTimeRange(tenantId, clusterId string,
	start, end, stepSeconds int64) (*types.ClusterNamespaceCostList, error) {
	...
	nsCost := make(map[string]map[int64]*types.ClusterNamespaceCostDetail)
	...
	ret := &types.ClusterNamespaceCostList{ClusterId: clusterId, Items: []*types.ClusterNamespaceCost{}}
	ret.Items = append(ret.Items, nsCosts...)
	return ret, nil
}

func parseNamepsaceTotalCosts(nsCost map[string]map[int64]*types.ClusterNamespaceCostDetail, totalCosts map[string]map[int64]float64) {
	for ns, details := range totalCosts {
		nsCost[ns] = make(map[int64]*types.ClusterNamespaceCostDetail)
		for timeStamp, v := range details {
			nsCost[ns][timeStamp] = &types.ClusterNamespaceCostDetail{
				Timestamp: timeStamp,
				TotalCost: v,
			}
		}
...
func parseNamespacePodCount(nsCost map[string]map[int64]*types.ClusterNamespaceCostDetail,
	...
	for ns, details := range podCount {
		...
		for timeStamp, v := range details {
			if _, ok := item[timeStamp]; !ok {
				item[timeStamp] = &types.ClusterNamespaceCostDetail{
					Timestamp: timeStamp,
				}
			}
...
func parseNamespaceResourceRequest(nsCost map[string]map[int64]*types.ClusterNamespaceCostDetail,
...
func parseNamespaceResourceUsage(nsCost map[string]map[int64]*types.ClusterNamespaceCostDetail,
...
func convertClusterNSCostToList(nsCost map[string]map[int64]*types.ClusterNamespaceCostDetail) []*types.ClusterNamespaceCost {
	ret := []*types.ClusterNamespaceCost{}
	for nsKey, details := range nsCost {
		cost := &types.ClusterNamespaceCost{
			Namespace: nsKey,
			CostList:  []*types.ClusterNamespaceCostDetail{},
		}
